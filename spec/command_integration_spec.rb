...
describe "commands" do
  before do
    ...
    @bs1 = "Pad.bs1 = _debunker_.binding_stack.dup"
    @bs2 = "Pad.bs2 = _debunker_.binding_stack.dup"
    @bs3 = "Pad.bs3 = _debunker_.binding_stack.dup"

    @self  = "Pad.self = self"

    @command_tester = Debunker::CommandSet.new do
  ...
  describe "alias_command" do
    it 'should make an aliasd command behave like its original' do
      ...
      debunker_tester(:commands => set).tap do |t|
    ...
    it 'should pass option arguments to original' do
      ...
      obj = Class.new { @x = 10 }
      t = debunker_tester(obj, :commands => set)
    ...
    it 'should be able to make the alias a regex' do
      ...
      redirect_debunker_io(InputTester.new("test-alias"), out1 = StringIO.new) do
        Debunker.start self, :commands => set
      end
  ...
  describe "Debunker::Command#run" do
    it 'should allow running of commands with following whitespace' do
      ...
      redirect_debunker_io(InputTester.new("cd 1/2/3/4/5/6", @bs1, "test-run",
                                      @self, @bs2, "exit-all")) do
        Debunker.start(@o, :commands => set)
      end
    ...
    it 'should run a command from within a command' do
      klass = Debunker::CommandSet.new do
        command "v" do
      ...
      expect(debunker_tester(:commands => klass).eval('run_v')).to match(/v command/)
    end
    ...
    it 'should run a regex command from within a command' do
      ...
      expect(debunker_tester(:commands => klass).eval('run_v')).to match(/v baby/)
    end
    ...
    it 'should run a command from within a command with arguments' do
      ...
      ["run_v_explicit_parameter", "run_v_embedded_parameter"].each do |cmd|
        expect(debunker_tester(:commands => klass).eval(cmd)).to match(/v baby param/)
      end
    end
  end

  describe "Debunker#run_command" do
    it 'should run a command that modifies the passed in eval_string' do
      p = Debunker.new(:output => @str_output)
      p.eval "def hello\npeter pan\n"
    ...
    it 'should run a command in the context of a session' do
      debunker_tester(Object.new).tap do |t|
        t.eval "@session_ivar = 10", "_debunker_.run_command('ls')"
  ...
  it 'should interpolate ruby code into commands' do
    ...
    expect(debunker_tester(:commands => set).eval('hello #{Pad.bong}')).to match(/bong/)
  end

  # bug fix for https://github.com/debunker/debunker/issues/170
  it 'should not choke on complex string interpolation when checking if ruby code is a command' do
    redirect_debunker_io(InputTester.new('/#{Regexp.escape(File.expand_path("."))}/'), @str_output) do
      debunker
    end
  ...
  it 'should NOT interpolate ruby code into commands if :interpolate => false' do
    ...
    expect(debunker_tester(:commands => set).eval('hello #{Pad.bong}')).
      to match(/Pad\.bong/)
  end

  it 'should NOT try to interpolate pure ruby code (no commands) ' do
    # These should raise RuntimeError instead of NameError
    expect { debunker_eval 'raise \'#{aggy}\'' }.to raise_error RuntimeError
    expect { debunker_eval 'raise #{aggy}'     }.to raise_error RuntimeError

    expect(debunker_eval('format \'#{my_var}\'')).to eq "\#{my_var}"
  end
  ...
  it 'should create a command with a space in its name zzz' do
    ...
    expect(debunker_tester(:commands => set).eval('hello baby')).
  ...
  it 'should create a command with a space in its name and pass an argument' do
    ...
    expect(debunker_tester(:commands => set).eval('hello baby john')).
  ...
  it 'should create a regex command and be able to invoke it' do
    ...
    expect(debunker_tester(:commands => set).eval('hello1')).to match(/hello1/)
  end
  ...
  it 'should create a regex command and pass captures into the args list before regular arguments' do
    ...
    expect(debunker_tester(:commands => set).eval('hello1 baby')).to match(/hello 1 baby/)
  end
  ...
  it 'should create a regex command and interpolate the captures' do
    ...
    bong = "bong"
    expect(debunker_tester(binding, :commands => set).eval('hello #{bong}')).
      to match(/hello #{bong}/)
  end
  ...
  it 'should create a regex command and arg_string should be interpolated' do
    ...
    expect(debunker_tester(binding, :commands => set).
  ...
  it 'if a regex capture is missing it should be nil' do
    ...
    expect(debunker_tester(:commands => set).eval('hello baby')).to match(/hello nil baby/)
  end

  it 'should create a command in a nested context and that command should be accessible from the parent' do
    expect(debunker_tester(Object.new).eval(*(<<-RUBY.split("\n")))).to match(/instance variables:\s+@x/m)
      @x = nil
      cd 7
      _debunker_.commands.instance_eval { command('bing') { |arg| run arg } }
      cd ..
      bing ls
    RUBY
  end
  ...
  it 'should define a command that keeps its return value' do
    ...
    t = debunker_tester(:commands => klass)
    t.eval("hello\n")
  ...
  it 'should define a command that does NOT keep its return value' do
    ...
    t = debunker_tester(:commands => klass)
    expect(t.eval("hello\n")).to eq ''
    expect(t.last_command_result).to eq Debunker::Command::VOID_VALUE
  end
  ...
  it 'should define a command that keeps its return value but does not return when value is void' do
    ...
    expect(debunker_tester(:commands => klass).eval("hello\n").empty?).to eq true
  end
  ...
  it 'a command (with :keep_retval => false) that replaces eval_string with a valid expression should not have the expression value suppressed' do
    ...
    redirect_debunker_io(InputTester.new('def yo', 'hello'), @str_output) do
      Debunker.start self, :commands => klass
    end
  ...
  it 'a command (with :keep_retval => true) that replaces eval_string with a valid expression should overwrite the eval_string with the return value' do
    ...
    expect(debunker_tester(:commands => klass).eval("def yo\nhello\n")).to eq 7
  end
  ...
  it 'should set the commands default, and the default should be overridable' do
    ...
    other_klass = Debunker::CommandSet.new do
      command "goodbye", "" do
        output.puts "goodbye world"
      end
    end

    Debunker.config.commands = klass
    expect(debunker_tester.eval("hello")).to eq "hello world\n"
    expect(debunker_tester(:commands => other_klass).eval("goodbye")).to eq "goodbye world\n"
  end

  it 'should inherit commands from Debunker::Commands' do
    klass = Debunker::CommandSet.new Debunker::Commands do
      command "v" do
      end
    end
  ...
  it 'should enable an inherited method to access opts and output and target, due to instance_exec' do
    ...
    child_klass = Debunker::CommandSet.new klass do
    end

    mock_debunker(Debunker.binding_for('john'), "v", :print => proc {}, :commands => child_klass,
                                           :output => @str_output)
  ...
  it 'should import commands from another command object' do
    klass = Debunker::CommandSet.new do
      import_from Debunker::Commands, "ls", "jump-to"
    end
  ...
  it 'should run a command with no parameter' do
    expect(debunker_tester(:commands => @command_tester).eval('command1')).
      to eq "command1\n"
  end

  it 'should run a command with one parameter' do
    expect(debunker_tester(:commands => @command_tester).eval('command2 horsey')).
      to eq "horsey\n"
  end
end
