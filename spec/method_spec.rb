...
describe Debunker::Method do
  ...
  describe ".from_str" do
    ...
    it 'should look up instance methods if "instance-methods"  option provided' do
      klass = Class.new { def hello; end; def self.hello; end  }
      meth = Debunker::Method.from_str(:hello, Debunker.binding_for(klass), {"instance-methods" => true})
      expect(meth).to eq klass.instance_method(:hello)
    end

    it 'should look up methods if :methods  option provided' do
      klass = Class.new { def hello; end; def self.hello; end  }
      meth = Debunker::Method.from_str(:hello, Debunker.binding_for(klass), {:methods => true})
      expect(meth).to eq klass.method(:hello)
    end

    it 'should look up instance methods using the Class#method syntax' do
      klass = Class.new { def hello; end; def self.hello; end  }
      meth = Debunker::Method.from_str("klass#hello", Debunker.binding_for(binding))
      expect(meth).to eq klass.instance_method(:hello)
    end
    ...
    it 'should NOT look up instance methods using the Class#method syntax if no instance methods defined' do
      ...
      meth = Debunker::Method.from_str("_klass#hello", Debunker.binding_for(binding))
    ...
    it 'should look up instance methods using klass.meth#method syntax' do
      ...
      meth = Debunker::Method.from_str("_klass.meth#initialize", Debunker.binding_for(binding))
      expect(meth.name).to eq "initialize"
    end

    it 'should look up methods using instance::bar syntax' do
      _klass = Class.new{ def self.meth; Class.new; end }
      meth = Debunker::Method.from_str("_klass::meth", Debunker.binding_for(binding))
      expect(meth.name).to eq "meth"
    end

    it 'should not raise an exception if receiver does not exist' do
      expect { Debunker::Method.from_str("random_klass.meth", Debunker.binding_for(binding)) }.to_not raise_error
    end
  end

  describe '.from_binding' do
    it 'should be able to pick a method out of a binding' do
      expect(Debunker::Method.from_binding(Class.new{ def self.foo; binding; end }.foo).name).to eq "foo"
    end

    it 'should NOT find a method from the toplevel binding' do
      expect(Debunker::Method.from_binding(TOPLEVEL_BINDING)).to eq nil
    end
    ...
    it "should find methods that have been undef'd" do
      ...
      m = Debunker::Method.from_binding(c.bar)
    ...
    unless Debunker::Helpers::BaseHelpers.rbx?
       it 'should find the super method correctly' do
        ...
        g = b.new.gag33
        m = Debunker::Method.from_binding(g)
    ...
    it 'should find the right method if a super method exists' do
      ...
      m = Debunker::Method.from_binding(b.new.gag)
    ...
    unless Debunker::Helpers::BaseHelpers.rbx?
      it "should find the right method from a BasicObject" do
        ...
        m = Debunker::Method.from_binding(a.new.gag)
        expect(m.owner).to eq a
    ...
    it 'should find the right method even if it was renamed and replaced' do
      ...
      m = Debunker::Method.from_binding(o.borscht)
      expect(m.source).to eq Debunker::Method(o.method(:paella)).source
    end
  end
  ...
  describe 'super' do
    it 'should be able to find the super method on a bound method' do
      ...
      zuper = Debunker::Method(obj.method(:rar)).super
    ...
    it 'should be able to find the super method of an unbound method' do
      ...
      zuper = Debunker::Method(b.instance_method(:rar)).super
      expect(zuper.owner).to eq a
    end
    ...
    it 'should return nil if no super method exists' do
      ...
      expect(Debunker::Method(a.instance_method(:rar)).super).to eq nil
    end
    ...
    it 'should be able to find super methods defined on modules' do
      ...
      zuper = Debunker::Method(a.new.method(:rar)).super
    ...
    it 'should be able to find super methods defined on super-classes when there are modules in the way' do
      ...
      zuper = Debunker::Method(b.new.method(:rar)).super
      expect(zuper.owner).to eq a
    end
  ...
  describe 'all_from_class' do
    def should_find_method(name)
      expect(Debunker::Method.all_from_class(@class).map(&:name)).to include name
    end
    ...
    it 'should attribute overridden methods to the sub-class' do
      ...
      expect(Debunker::Method.all_from_class(@class).detect{ |x| x.name == 'meth' }.owner).to eq @class
    end
  ...
  describe 'all_from_obj' do
    describe 'on normal objects' do
      def should_find_method(name)
        expect(Debunker::Method.all_from_obj(@obj).map(&:name)).to include name
      end
      ...
      it "should not find methods defined on the classes singleton class" do
        ...
        expect(Debunker::Method.all_from_obj(@obj).map(&:name)).not_to include 'meth'
      end
    ...
    describe 'on classes' do
      def should_find_method(name)
        expect(Debunker::Method.all_from_obj(@class).map(&:name)).to include name
      end
      ...
      it "should not find methods defined within the class" do
        @class = Class.new{ def meth; 1; end }
        expect(Debunker::Method.all_from_obj(@class).map(&:name)).not_to include 'meth'
      end
      ...
      it "should attribute overridden methods to the sub-class' singleton class" do
        ...
        expect(Debunker::Method.all_from_obj(@class).detect{ |x| x.name == 'meth' }.owner).to eq(class << @class; self; end)
      end
      ...
      it "should attribute overridden methods to the relevant singleton class in preference to Class" do
        ...
        expect(Debunker::Method.all_from_obj(@class).detect{ |x| x.name == 'allocate' }.owner).to eq(class << @class; self; end)
      end
    end
    ...
    describe 'method resolution order' do
      ...
      it "should look at a class and then its superclass" do
        ...
        expect(Debunker::Method.instance_resolution_order(LS::Next)).to eq [LS::Next] + Debunker::Method.instance_resolution_order(LS::Top)
      end

      it "should include the included modules between a class and its superclass" do
        expect(Debunker::Method.instance_resolution_order(LS::Low)).to eq [LS::Low, LS::P, LS::N, LS::M] + Debunker::Method.instance_resolution_order(LS::Next)
      end

      it "should not include modules extended into the class" do
        expect(Debunker::Method.instance_resolution_order(LS::Bottom)).to eq [LS::Bottom] + Debunker::Method.instance_resolution_order(LS::Lower)
      end

      it "should include included modules for Modules" do
        expect(Debunker::Method.instance_resolution_order(LS::O)).to eq [LS::O, LS::M]
      end

      it "should include the singleton class of objects" do
        obj = LS::Low.new
        expect(Debunker::Method.resolution_order(obj)).to eq [eigen_class(obj)] + Debunker::Method.instance_resolution_order(LS::Low)
      end

      it "should not include singleton classes of numbers" do
        expect(Debunker::Method.resolution_order(4)).to eq Debunker::Method.instance_resolution_order(Fixnum)
      end

      it "should include singleton classes for classes" do
        expect(Debunker::Method.resolution_order(LS::Low)).to eq [eigen_class(LS::Low)] + Debunker::Method.resolution_order(LS::Next)
      end

      it "should include modules included into singleton classes" do
        expect(Debunker::Method.resolution_order(LS::Lower)).to eq [eigen_class(LS::Lower), LS::N, LS::M] + Debunker::Method.resolution_order(LS::Low)
      end

      it "should include modules at most once" do
        expect(Debunker::Method.resolution_order(LS::Bottom).count(LS::M)).to eq 1
      end

      it "should include modules at the point which they would be reached" do
        expect(Debunker::Method.resolution_order(LS::Bottom)).to eq [eigen_class(LS::Bottom), LS::O] + (Debunker::Method.resolution_order(LS::Lower))
      end

      it "should include the Debunker::Method.instance_resolution_order of Class after the singleton classes" do
        expect(Debunker::Method.resolution_order(LS::Top)).to eq(
          [eigen_class(LS::Top), eigen_class(Object), eigen_class(BasicObject),
           *Debunker::Method.instance_resolution_order(Class)]
        )
      end
    end
  end
  ...
  describe 'method aliases' do
    ...
    it 'should be able to find method aliases' do
      meth = Debunker::Method(@class.new.method(:eat))
    ...
    it 'should return an empty Array if cannot find aliases' do
      meth = Debunker::Method(@class.new.method(:eruct))
    ...
    it 'should find aliases for top-level methods' do
      ...
      meth = Debunker::Method.new(method(:my_top_level_method))
    ...
    it 'should be able to find aliases for methods implemented in C' do
      meth = Debunker::Method(Hash.new.method(:key?))
      aliases = Set.new(meth.aliases)
