...
describe "show-source" do
  ...
  it "should output a method's source" do
    expect(debunker_eval(binding, 'show-source @o.sample_method')).to match(/def @o.sample/)
  end

  it "should output help" do
    expect(debunker_eval('show-source -h')).to match(/Usage:\s+show-source/)
  end

  it "should output a method's source with line numbers" do
    expect(debunker_eval(binding, 'show-source -l @o.sample_method')).to match(/\d+: def @o.sample/)
  end

  it "should output a method's source with line numbers starting at 1" do
    expect(debunker_eval(binding, 'show-source -b @o.sample_method')).to match(/1: def @o.sample/)
  end

  it "should output a method's source if inside method and no name given" do
    def @o.sample
      debunker_eval(binding, 'show-source').should =~ /def @o.sample/
    end
    @o.sample
  end
  ...
  it "should output a method's source inside method using the -l switch" do
    def @o.sample
      debunker_eval(binding, 'show-source -l').should =~ /def @o.sample/
    end
    @o.sample
  end
  ...
  it "should find methods even if there are spaces in the arguments" do
    ...
    out = debunker_eval(binding, "show-source @o.foo('bar', 'baz bam').foo")
  ...
  it "should find methods even if the object overrides method method" do
    ...
    expect(debunker_eval(binding, "show-source _c.new.method")).to match(/98/)
  end

  it "should not show the source when a non-extant method is requested" do
    _c = Class.new{ def method; 98; end }
    expect(mock_debunker(binding, "show-source _c#wrongmethod")).to match(/Couldn't locate/)
  end
  ...
  it "should find instance_methods if the class overrides instance_method" do
    ...
    expect(debunker_eval(binding, "show-source _c#method")).to match(/98/)
  end

  it "should find instance methods with self#moo" do
    _c = Class.new{ def moo; "ve over!"; end }

    expect(debunker_eval(binding, "cd _c", "show-source self#moo")).to match(/ve over/)
  end

  it "should not find instance methods with self.moo" do
    _c = Class.new{ def moo; "ve over!"; end }

    expect { debunker_eval(binding, 'cd _c', 'show-source self.moo') }.to raise_error(Debunker::CommandError, /Couldn't locate/)
  end
  ...
  it "should find normal methods (i.e non-instance methods) by default" do
    ...
    expect(debunker_eval(binding, "cd _c", "show-source moo")).to match(/ve over/)
  end
  ...
  it "should raise a CommandError when super method doesn't exist" do
    ...
    expect { debunker_eval(binding, "show-source --super @o.foo") }.to raise_error(Debunker::CommandError, /No superclass found/)
  end

  it "should output the source of a method defined inside Debunker" do
    out = debunker_eval("def dyn_method\n:test\nend", 'show-source dyn_method')
    expect(out).to match(/def dyn_method/)
    Object.remove_method :dyn_method
  end

  it 'should output source for an instance method defined inside debunker' do
  ...
  it "should output the source of a command defined inside Debunker" do
    command_definition = %{
      Debunker.config.commands.command "hubba-hubba" do
        puts "that's what she said!"
      end
    }
    out = debunker_eval(command_definition, 'show-source hubba-hubba')
    expect(out).to match(/what she said/)
    Debunker.config.commands.delete "hubba-hubba"
  end
  ...
  describe "finding super methods with help of `--super` switch" do
    ...
    it "finds super methods with explicit method argument" do
      ...
      expect(debunker_eval(binding, "show-source --super o.foo")).to match(/:super_wibble/)
    end
    ...
    it "finds super methods without explicit method argument" do
      ...
      def o.foo(*bars)
        @foo = :wibble
        debunker_eval(binding, 'show-source --super')
      end
    ...
    it "finds super methods with multiple --super " do
      ...
      def o.foo(*bars)
        @foo = :wibble
        debunker_eval(binding, 'show-source --super --super')
      end
  ...
  describe "on sourcable objects" do
    it "should output source defined inside debunker" do
      debunker_tester.tap do |t|
    ...
    it "should output source for procs/lambdas stored in variables" do
      ...
      expect(debunker_eval(binding, 'show-source _hello')).to match(/proc \{ puts/)
    end

    it "should output source for procs/lambdas stored in constants" do
      HELLO = proc { puts 'hello world!' }
      expect(debunker_eval(binding, "show-source HELLO")).to match(/proc \{ puts/)
    ...
    it "should output source for method objects" do
      ...
      _meth = @o.method(:hi)
      expect(debunker_eval(binding, "show-source _meth")).to match(/puts 'hi world'/)
    end

    describe "on variables that shadow methods" do
      before do
        @t = debunker_tester.eval unindent(<<-EOS)
          class ::TestHost
            def hello
              hello = proc { ' smile ' }
              _foo = hello
              debunker_tester(binding)
            end
          end
          ::TestHost.new.hello
          EOS
    ...
    describe "on variable or constant" do
      ...
      it "should output source of its class if variable doesn't respond to source_location" do
        ...
        expect(debunker_eval(binding, 'show-source _test_host')).
      ...
      it "should output source of its class if constant doesn't respond to source_location" do
        ...
        expect(debunker_eval(binding, 'show-source TEST_HOST')).
          to match(/class TestHost\n.*def hello/)
    ...
    describe "on modules" do
      ...
      describe "basic functionality, should find top-level module definitions" do
        ...
        it 'should show source for a super class when Const = Class.new syntax is used' do
          expect(debunker_eval('show-source -s ShowSourceTestClassWeirdSyntax')).
            to match(/class Object/)
        end

        it 'should show source for a module when Const = Module.new syntax is used' do
          expect(debunker_eval('show-source ShowSourceTestModuleWeirdSyntax')).
            to match(/ShowSourceTestModuleWeirdSyntax = Module.new/)
        end
      end

      before do
        debunker_eval unindent(<<-EOS)
        class Dog
          def woof
          end
        end
      ...
      describe "in REPL" do
        it 'should find class defined in repl' do
          expect(debunker_eval('show-source TobinaMyDog')).to match(/class TobinaMyDog/)
        end

        it 'should find superclass defined in repl' do
          expect(debunker_eval('show-source -s TobinaMyDog')).to match(/class Dog/)
        end
      end
      ...
      it 'should lookup module name with respect to current context' do
        ...
        constant_scope(:AlphaClass, :BetaClass) do
          ...
          expect(debunker_eval(AlphaClass, 'show-source BetaClass')).to match(/def beta/)
        end
      end
      ...
      it 'should lookup nested modules' do
        constant_scope(:AlphaClass) do
          ...
          expect(debunker_eval('show-source AlphaClass::BetaClass')).to match(/class Beta/)
        end
      end

      # note that debunker assumes a class is only monkey-patched at most
      ...
      describe "show-source -a" do
        it 'should show the source for all monkeypatches defined in different files' do
          ...
          result = debunker_eval('show-source TestClassForShowSource -a')
        ...
        it 'should show the source for a class_eval-based monkeypatch' do
          ...
          result = debunker_eval('show-source TestClassForShowSourceClassEval -a')
        ...
        it 'should ignore -a when object is not a module' do
          ...
          result = debunker_eval('show-source TestClassForShowSourceClassEval#class_eval_method -a')
        ...
        it 'should show the source for an instance_eval-based monkeypatch' do
          ...
          result = debunker_eval('show-source TestClassForShowSourceInstanceEval -a')
        ...
        describe "messages relating to -a" do
          it 'indicates all available monkeypatches can be shown with -a when (when -a not used and more than one candidate exists for class)' do
            ...
            result = debunker_eval('show-source TestClassForShowSource')
          ...
          it 'shouldnt say anything about monkeypatches when only one candidate exists for selected class' do
            ...
            result = debunker_eval('show-source Aarrrrrghh')
      ...
      describe "when show-source is invoked without a method or class argument" do
          ...
          module TestHost
            ...
            module D
              def self.invoked_in_method
                debunker_eval(binding, 'show-source')
              end
            end
          end
        end
        ...
        describe "inside a module" do
          it 'should display module source by default' do
            out = debunker_eval(TestHost::M, 'show-source')
          ...
          it 'should be unable to find module source if no methods defined' do
            expect { debunker_eval(TestHost::C, 'show-source') }.to raise_error(Debunker::CommandError, /Couldn't locate/)
          end

          it 'should display method code (rather than class) if Debunker started inside method binding' do
          ...
          it 'should display class source when inside instance' do
            out = debunker_eval(TestHost::M.new, 'show-source')
            expect(out).to match(/class M/)
            expect(out).to match(/def alpha/)
            expect(out).to match(/def beta/)
          end
          ...
          it 'should allow options to be passed' do
            out = debunker_eval(TestHost::M, 'show-source -b')
          ...
          describe "should skip over broken modules" do
            ...
            it 'should return source for first valid module' do
              out = debunker_eval('show-source BabyDuck::Muesli')
          ...
          describe "monkey-patched C modules" do
            ...
            describe "when current context is a C object" do
              it "should display a warning, and not monkey-patched definition" do
                out = debunker_eval([1, 2, 3], 'show-source')
            ...
            describe "when current context is something other than a C object" do
              it "should display a candidate, not a warning" do
                out = debunker_eval('show-source Array')
    ...
    describe "on commands" do
      ...
      describe "create_command commands" do
        ...
        it 'should show source for a command defined inside debunker' do
          debunker_eval %{
          _debunker_.commands.create_command "foo", "babble" do
            def process() :body_of_foo end
          end
        }
        expect(debunker_eval('show-source foo')).to match(/:body_of_foo/)
    ...
    describe "real class-based commands" do
      before do
        class ::TemporaryCommand < Debunker::ClassCommand
          match 'temp-command'
          def process() :body_of_temp end
        end

        Debunker.config.commands.add_command(::TemporaryCommand)
      end
      ...
      it 'should show source for a command' do
        expect(debunker_eval('show-source temp-command')).to match(/:body_of_temp/)
      end

      it 'should show source for a command defined inside debunker' do
        debunker_eval %{
          class ::TemporaryCommandInDebunker < Debunker::ClassCommand
            match 'temp-command-in-debunker'
            def process() :body_of_temp end
          end
        }
        Debunker.config.commands.add_command(::TemporaryCommandInDebunker)
        expect(debunker_eval('show-source temp-command-in-debunker')).to match(/:body_of_temp/)
        Object.remove_const(:TemporaryCommandInDebunker)
      end
    end
  end
    ...
    describe "can't find class/module code" do
      describe "for classes" do
        ...
        it 'errors when class has no superclass to show' do
          t = debunker_tester
          expect { t.process_command "show-source Jesus::Brian" }.to raise_error(Debunker::CommandError, /Couldn't locate/)
        end
