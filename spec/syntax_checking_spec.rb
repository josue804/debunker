...
describe Debunker do
  ...
  [
    ...
    it "should not raise an error on broken lines: #{foo.join("\\n")}" do
      redirect_debunker_io(InputTester.new(*foo), @str_output) do
        Debunker.start
      end
  ...
  ([
    ...
    ["1 1"],
    ["puts :"]
  ] + (Debunker::Helpers::BaseHelpers.rbx? ? [] : [
  ...
  it "should not intefere with syntax errors explicitly raised" do
    redirect_debunker_io(InputTester.new(%q{raise SyntaxError, "unexpected $end"}), @str_output) do
      Debunker.start
    end
  ...
  it "should allow trailing , to continue the line" do
    expect(Debunker::Code.complete_expression?("puts 1, 2,")).to eq false
  end

  it "should complete an expression that contains a line ending with a ," do
    expect(Debunker::Code.complete_expression?("puts 1, 2,\n3")).to eq true
  end

  it "should not suppress the error output if the line ends in ;" do
    expect(mock_debunker("raise RuntimeError, 'foo';")).to match(/RuntimeError/)
  end

  it "should not clobber _ex_ on a SyntaxError in the repl" do
    expect(mock_debunker("raise RuntimeError, 'foo'", "puts foo)", "_ex_.is_a?(RuntimeError)")).to match(/^RuntimeError.*\nSyntaxError.*\n=> true/m)
  end

  it "should allow whitespace delimeted strings" do
    expect(mock_debunker('"%s" % % foo ')).to match(/"foo"/)
  end
  ...
  it "should allow whitespace delimeted strings ending on the first char of a line" do
    expect(mock_debunker('"%s" % % ', ' #done!')).to match(/"\\n"/)
  end
end
