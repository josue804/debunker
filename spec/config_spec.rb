...
describe Debunker::Config do
  ...
  describe "reserved keys" do
    it "raises Debunker::Config::ReservedKeyError on assignment of a reserved key" do
      local = Debunker::Config.new
      local.instance_variable_get(:@reserved_keys).each do |key|
        expect { local[key] = 1 }.to raise_error(Debunker::Config::ReservedKeyError)
      end
    end
  end
  ...
  describe "traversal to parent" do
    it "traverses back to the parent when a local key is not found" do
      local = Debunker::Config.new Debunker::Config.from_hash(foo: 1)
    ...
    it "traverses through a chain of parents" do
      root = Debunker::Config.from_hash({foo: 21})
      local1 = Debunker::Config.new(root)
      local2 = Debunker::Config.new(local1)
      local3 = Debunker::Config.new(local2)
      expect(local3.foo).to eq(21)
    end

    it "stores a local copy of the parent's hooks upon accessing them" do
      parent = Debunker::Config.from_hash(hooks: "parent_hooks")
      local  = Debunker::Config.new parent
  ...
  describe ".from_hash" do
    it "returns an object without a default" do
      local = Debunker::Config.from_hash({})
      expect(local.default).to eq(nil)
    end

    it "returns an object with a default" do
      default = Debunker::Config.new(nil)
      local = Debunker::Config.from_hash({}, default)
      expect(local.default).to eq(local)
    end

    it "recursively builds Debunker::Config objects from a Hash" do
      h = {
        'foo1' => {
          'foo2' => {
            'foo3' => 'foobar'
      ...
      default = Debunker::Config.from_hash(h)
  ...
  describe "#respond_to_missing?" do
    before do
      @config = Debunker::Config.new(nil)
    end
  ...
  describe "#default" do
    it "returns nil" do
      local = Debunker::Config.new(nil)
      expect(local.default).to eq(nil)
    end

    it "returns the default" do
      default = Debunker::Config.new(nil)
      local = Debunker::Config.new(default)
  ...
  describe "#keys" do
    it "returns an array of local keys" do
      root = Debunker::Config.from_hash({zoo: "boo"}, nil)
      local = Debunker::Config.from_hash({foo: "bar"}, root)
  ...
  describe "#==" do
    it "compares equality through the underlying lookup table" do
      local1 = Debunker::Config.new(nil)
      local2 = Debunker::Config.new(nil)
      local1.foo = "hi"
      local2.foo = "hi"
  ...
  describe "#to_hash" do
    it "provides a copy of local key & value pairs as a Hash" do
      local = Debunker::Config.new Debunker::Config.from_hash(bar: true)
      local.foo = "21"
  ...
  describe "#[]" do
    it "traverses back to a default" do
      default = Debunker::Config.from_hash({k: 1})
      local = Debunker::Config.new(default)
      expect(local['k']).to eq(1)
    end

    it "traverses back to a default (2 deep)" do
      default1 = Debunker::Config.from_hash({k: 1})
      default2 = Debunker::Config.from_hash({}, default1)
      local = Debunker::Config.new(default2)
      expect(local['k']).to eq(1)
    end

    it "traverses back to a default that doesn't exist, and returns nil" do
      local = Debunker::Config.from_hash({}, nil)
