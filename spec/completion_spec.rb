...
def completer_test(bind, debunker=nil, assert_flag=true)
  test = proc {|symbol|
    expect(Debunker::InputCompleter.new(debunker || Readline, debunker).call(symbol[0..-2], :target => Debunker.binding_for(bind)).include?(symbol)).to  eq(assert_flag)}
  return proc {|*symbols| symbols.each(&test) }
end


describe Debunker::InputCompleter do
  before do
    # The AMQP gem has some classes like this:
    #  debunker(main)> AMQP::Protocol::Test::ContentOk.name
    #  => :content_ok
    module SymbolyName
      def self.name; :symboly_name; end
    end

    @before_completer = Debunker.config.completer
    Debunker.config.completer = Debunker::InputCompleter
  end

  after do
    Debunker.config.completer = @before_completer
  ...
  if !Debunker::Helpers::BaseHelpers.jruby?
    it "should not crash if there's a Module that has a symbolic name." do
      expect { Debunker::InputCompleter.new(Readline).call "a.to_s.", :target => Debunker.binding_for(Object.new) }.not_to raise_error
    end
  end

  it 'should take parenthesis and other characters into account for symbols' do
    expect { Debunker::InputCompleter.new(Readline).call ":class)", :target => Debunker.binding_for(Object.new) }.not_to raise_error
  end
  ...
  it 'should complete instance variables' do
    ...
    object.instance_variable_set(:'@name', 'Debunker')
    ...
    b = Debunker.binding_for(object)
    completer_test(b).call('@name', '@name.downcase')

    # Complete class variables.
    b = Debunker.binding_for(object.class)
  ...
  it 'should complete for target symbols' do
    ...
    # trailing slash
    expect(Debunker::InputCompleter.new(Readline).call('Mod2/', :target => Debunker.binding_for(Mod)).include?('Mod2/')).to   eq(true)
  end
  ...
  it 'should complete for arbitrary scopes' do
    ...
    debunker = Debunker.new(:target => Baz)
    debunker.push_binding(Bar)

    b = Debunker.binding_for(Bar)
    completer_test(b, debunker).call("../@bazvar")
    completer_test(b, debunker).call('/Con')
  end
  ...
  it 'should not return nil in its output' do
    ...
    expect(Debunker::InputCompleter.new(Readline, debunker).call("debunker.", :target => binding)).not_to include nil
  end
end
