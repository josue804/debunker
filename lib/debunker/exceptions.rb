class Debunker
  ...
  # from exiting Debunker when they want to.
  module RescuableException
    def self.===(exception)
      ...
      when Interrupt
        ...
        # to be intended for debunker itself. We should also make sure that
        # Kernel#exit works.
      when *Debunker.config.exception_whitelist
        false
  ...
  module Debunker::TooSafeException
    def self.===(exception)
      $SAFE > 0 && SecurityError === exception
    end
  end

  # An Exception Tag (cf. Exceptional Ruby) that instructs Debunker to show the error
  # in a more user-friendly manner. This should be used when the exception
  # happens within Debunker itself as a direct consequence of the user typing
  # something wrong.
  #
  ...
  #
  # debunker(main)> method_that_evals("def )")
