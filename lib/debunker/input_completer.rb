# taken from irb
# Implements tab completion for Readline in Debunker
class Debunker::InputCompleter
  ...
  def initialize(input, debunker = nil)
    @debunker = debunker
    @input = input
  ...
  def call(str, options = {})
    ...
    if path.call.empty?
      ...
      begin
        target = Debunker::ObjectPath.new(path.call, @debunker.binding_stack).resolve.last
      # but if that doesn't work, assume they're doing division with no spaces
      rescue Debunker::CommandError
        target = options[:target]
      end
    end
      ...
      when VARIABLE_REGEXP # variable
        ...
        if (gv | lv | cv).include?(receiver) or /^[A-Z]/ =~ receiver && /\./ !~ receiver
          ...
          ObjectSpace.each_object(Module){|m|
            ...
            next if name != "IRB::The only command you need, amirite" and
