...
class Debunker
  class << self
    # If the given object is a `Debunker::Method`, return it unaltered. If it's
    # anything else, return it wrapped in a `Debunker::Method` instance.
    def Method(obj)
      if obj.is_a? Debunker::Method
        obj
      else
        Debunker::Method.new(obj)
      end
    end
  end

  # This class wraps the normal `Method` and `UnboundMethod` classes
  # to provide extra functionality useful to Debunker.
  class Method
    require 'debunker/method/weird_method_locator'
    require 'debunker/method/disowned'
    require 'debunker/method/patcher'
    ...
    class << self
      ...
      # search in, find and return the requested method wrapped in a `Debunker::Method`
      # instance.
      #
      ...
      #   contain any context.
      # @return [Debunker::Method, nil] A `Debunker::Method` instance containing the requested
      ...
      # use it to instantiate a `Debunker::Method`. Return `nil` if this isn't
      # possible.
      #
      # @param [Binding] b
      # @return [Debunker::Method, nil]
      #
      def from_binding(b)
      ...
      def lookup_method_via_binding(obj, method_name, method_type, target=TOPLEVEL_BINDING)
        Debunker.current[:obj] = obj
        Debunker.current[:name] = method_name
        receiver = obj.is_a?(Module) ? "Module" : "Kernel"
        target.eval("::#{receiver}.instance_method(:#{method_type}).bind(Debunker.current[:obj]).call(Debunker.current[:name])")
      ensure
        Debunker.current[:obj] = Debunker.current[:name] = nil
      end

      # Given a `Class` or `Module` and the name of a method, try to
      # instantiate a `Debunker::Method` containing the instance method of
      ...
      # a `Debunker::Method` containing the method of that name bound to
      ...
      def instance_resolution_order(klass)
        ...
        ([klass] + Debunker::Method.safe_send(klass, :ancestors)).uniq
      end
      ...
      def singleton_class_resolution_order(klass)
        ancestors = Debunker::Method.safe_send(klass, :ancestors)
    ...
    # Get the owner of the method as a Debunker::Module
    # @return [Debunker::Module]
    def wrapped_owner
      @wrapped_owner ||= Debunker::WrappedModule.new(owner)
    end

    # Get underlying object wrapped by this Debunker::Method instance
    ...
    def redefine(source)
      ...
      Debunker::Method(owner.instance_method(name))
    end
    ...
    # @return [Debunker::Method, nil] The wrapped method that is called when you
    #   use "super" in the body of this method.
    def super(times=1)
      if UnboundMethod === @method
        sup = super_using_ancestors(Debunker::Method.instance_resolution_order(owner), times)
      else
        sup = super_using_ancestors(Debunker::Method.resolution_order(receiver), times)
        sup &&= sup.bind(receiver)
      end
      Debunker::Method.new(sup) if sup
    end
    ...
    # @return [Boolean] Was the method defined within the Debunker REPL?
    def debunker_method?
      source_file == Debunker.eval_path
    end
    ...
    # @return [Boolean]
    def is_a?(klass)
      klass == Debunker::Method or @method.is_a?(klass)
    end
    alias kind_of? is_a?
    ...
    def comment
      Debunker::Code.from_file(source_file).comment_describing(source_line)
    end

    private

    # @return [YARD::CodeObjects::MethodObject]
    # @raise [CommandError] when the method can't be found or `debunker-doc` isn't installed.
    def debunker_doc_info
      if Debunker.config.has_debunker_doc
        Debunker::MethodInfo.info_for(@method) or raise CommandError, "Cannot locate this method: #{name}. (source_location returns nil)"
      else
        fail_msg = "Cannot locate this method: #{name}."
        if mri?
          fail_msg += " Invoke the 'gem-install debunker-doc' Debunker command to get access to Ruby Core documentation.\n"
        end
    ...
    def ruby_source
      ...
      # our input buffer for methods defined in (debunker)
      file, line = *source_location
      raise SourceNotFoundError, "Could not locate source for #{name_with_owner}!" unless file

      begin
        code = Debunker::Code.from_file(file).expression_at(line)
