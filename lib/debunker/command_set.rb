class Debunker
  ...
  class CommandSet
    ...
    # Defines a new Debunker command.
    ...
    #   `Debunker.config.command_prefix` prefix (if one is defined). Defaults
    #   to true.
    ...
    #   command parameters are separated by whitespace at the Debunker prompt.
    # @example
    #   MyCommands = Debunker::CommandSet.new do
    ...
    #
    #   # From debunker:
    #   # debunker(main)> _debunker_.commands = MyCommands
    #   # debunker(main)> greet john
    #   # Good afternoon John!
    #   # debunker(main)> help greet
    #   # Greet somebody
    ...
    #   # From debunker:
    #   # debunker(main)> _debunker_.commands = MyCommands
    #   # debunker(main)> number-10 john
    #   # hello john, nice number: 10
    #   # debunker(main)> help number
    #   # number-N regex command
    def block_command(match, description="No description.", options={}, &block)
      #josue description, options = ["No description.", description] if description.is_a?(Hash)
      #josue options = Debunker::Command.default_options(match).merge!(options)

      #josue @commands[match] = Debunker::BlockCommand.subclass(match, description, options, helper_module, &block)
    end
    alias_method :command, :block_command

    # Defines a new Debunker command class.
    #
    # @param [String, Regexp] match The start of invocations of this command.
    # @param [String] description A description of the command.
    ...
    #
    # @example
    #   Debunker::Commands.create_command "echo", "echo's the input", :shellwords => false do
    #     def options(opt)
    ...
    #
    #     def process
    #       raise Debunker::CommandError, "-u and -d makes no sense" if opts.present?(:u) && opts.present?(:d)
    #       result = args.join(" ")
    ...
    def create_command(match, description="No description.", options={}, &block)
      ...
      #josue @commands[match] = Debunker::ClassCommand.subclass(match, description, options, helper_module, &block)
      #josue @commands[match].class_eval(&block)
      #josue @commands[match]
    end
    ...
    #   Debunker.config.commands.before_command("whereami") do |n|
    #     output.puts "parameter passed was #{n}"
    #   end
    # @deprecated Use {Debunker::Hooks#add_hook} instead.
    ...
    #   Debunker.config.commands.after_command("whereami") do |n|
    ...
    #   will be imported.
    # @return [Debunker::CommandSet] Returns the reciever (a command set).
    def import(*sets)
      sets.each do |set|
    ...
    def find_command_by_match_or_listing(match_or_listing)
      cmd = (@commands[match_or_listing] ||
        Debunker::Helpers::BaseHelpers.find_command(match_or_listing, @commands))
    ...
    #   Debunker.config.commands.alias_command "lM", "ls -M"
    # @example Pass explicit description (overriding default).
    #   Debunker.config.commands.alias_command "lM", "ls -M", :desc => "cutiepie"
    ...
    #   Debunker.config.commands.rename "dir", "ls", :description => "DOS friendly ls"
    ...
    #   end
    # @example Getting
    #   Debunker.config.commands.desc "amend-line"
    ...
    #
    # @param [Debunker::Command] command
    #   The command to add.
    #
    # @return [Debunker::Command]
    #   Returns the new command (matched with "pattern".)
    #
    # @example
    #   Debunker.config.commands["help"] = MyHelpCommand
    #
    def []=(pattern, command)
      if command.equal?(nil)
        return @commands.delete(pattern)
      end
      unless Class === command && command < Debunker::Command
        raise TypeError, "command is not a subclass of Debunker::Command"
      end
    ...
    #   a subclass of Debunker::Command.
    #
