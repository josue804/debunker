class Debunker
  class CodeFile
    ...
    EXTENSIONS = {
      ...
      %w(.rb .ru .irbrc .gemspec .debunkerrc .rake) => :ruby,
    }
    ...
    def code
      if @filename == Debunker.eval_path
        Debunker.line_buffer.drop(1)
      elsif Debunker::Method::Patcher.code_for(@filename)
        Debunker::Method::Patcher.code_for(@filename)
    ...
    # @return [Array] All the paths that contain code that Debunker can use for its
    #   API's. Skips directories.
    def code_path
      [from_pwd, from_debunker_init_pwd, *from_load_path]
    end
    ...
    # @return [String]
    def from_debunker_init_pwd
      File.expand_path(@filename, Debunker::INITIAL_PWD)
    end
