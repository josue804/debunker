class Debunker
  # @return [Array] Code of the method used when implementing Debunker's
...
class Object
  # Start a Debunker REPL on self.
  #
  # If `self` is a Binding then that will be used to evaluate expressions;
  # otherwise a new binding will be created.
  #
  # @param [Object] object  the object or binding to debunker
  #                         (__deprecated__, use `object.debunker`)
  ...
  #   def my_method
  #     binding.debunker :quiet => true
  #   end
  #   my_method()
  # @see Debunker.start
  def debunker(object=nil, hash={})
    if object.nil? || Hash === object
      Debunker.start(self, object || {})
    else
      Debunker.start(object, hash)
    end
  end
  ...
  def __binding__
    ...
    unless respond_to?(:__debunker__)
      ...
      # __debunker__ is just the closest method we have to hand, and using
      ...
      begin
        ...
        instance_eval(*Debunker::BINDING_METHOD_IMPL)
      ...
      rescue TypeError, Debunker::FrozenObjectException
        # class_eval sets the default definee to self.class
        self.class.class_eval(*Debunker::BINDING_METHOD_IMPL)
      end
    end
