class Debunker
  ...
  class History
    ...
    # @return [Fixnum] Number of lines in history when Debunker first loaded.
    ...
    def restore_default_behavior
      Debunker.config.input # force Readline to load if applicable
    ...
    def push(line)
      unless line.empty? || (@history.last && line == @history.last)
        @pusher.call(line)
        @history << line
        @saver.call(line) if Debunker.config.history.should_save
      end
      line
    end
    alias << push
    ...
    # The default loader. Yields lines from `Debunker.history.config.file`.
    def read_from_file
    ...
    # The default saver. Appends the given line to `Debunker.history.config.file`.
    ...
    def history_file_path
      File.expand_path(@file_path || Debunker.config.history.file)
    end
  end
end
