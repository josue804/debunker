require 'debunker/wrapped_module/candidate'

class Debunker
  class << self
    # If the given object is a `Debunker::WrappedModule`, return it unaltered. If it's
    # anything else, return it wrapped in a `Debunker::WrappedModule` instance.
    def WrappedModule(obj)
      if obj.is_a? Debunker::WrappedModule
        obj
      else
        Debunker::WrappedModule.new(obj)
      end
    end
  end
  ...
  class WrappedModule
    ...
    # @example
    #   Debunker::WrappedModule.from_str("Debunker::Code")
    def self.from_str(mod_name, target=TOPLEVEL_BINDING)
      if safe_to_evaluate?(mod_name, target)
        Debunker::WrappedModule.new(target.eval(mod_name))
      else
        nil
      end
    ...
    class << self
      ...
      # TODO: see duplication of this method in Debunker::CodeObject
    ...
    # @return [Boolean]
    def singleton_class?
      if Debunker::Method.safe_send(wrapped, :respond_to?, :singleton_class?)
        Debunker::Method.safe_send(wrapped, :singleton_class?)
        ...
        wrapped != Debunker::Method.safe_send(wrapped, :ancestors).first
      end
    end
    ...
    #Alias for `WrappedModule#candidate` to select the candidate you're
    # interested in.
    # @raise [Debunker::CommandError] If documentation cannot be found.
    ...
    #Alias for `WrappedModule#candidate` to select the candidate you're
    # interested in.
    # @raise [Debunker::CommandError] If source cannot be found.
    ...
    # @raise [Debunker::CommandError] If the `rank` is out of range. That
    #   is greater than `number_of_candidates - 1`.
    # @param [Fixnum] rank
    # @return [Debunker::WrappedModule::Candidate]
    def candidate(rank)
    ...
    def candidates
      ...
      Debunker::Helpers::BaseHelpers.jruby_19? ? enum.to_a : enum
    end
    ...
    # @return [Debunker::WrappedModule, nil] The wrapped module that is the
    #   superclass.
    ...
    def super(times=1)
      ...
      Debunker::WrappedModule(sup) if sup
    end

    private

    # @return [Debunker::WrappedModule::Candidate] The candidate with the
    ...
    # @return [Array<Array<Debunker::Method>>] The array of `Debunker::Method` objects,
    ...
    def all_methods_for(mod)
      Debunker::Method.all_from_obj(mod, false) + Debunker::Method.all_from_class(mod, false)
    end
    ...
    def lines_for_file(file)
      ...
      if file == Debunker.eval_path
        @lines_for_file[file] ||= Debunker.line_buffer.drop(1)
      else
