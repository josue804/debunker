  ...
  class PluginManager
    ...
    class Plugin
      ...
      def activate!
        ...
        Debunker.config.send("#{gem_name.gsub('-', '_')}=", Debunker::Config.from_hash({}))
      ...
      def supported?
        debunker_version = Gem::Version.new(VERSION)
        spec.dependencies.each do |dependency|
          if dependency.name == "debunker"
            return dependency.requirement.satisfied_by?(debunker_version)
          end
        end
        true
      end
    ...
    # Find all installed Debunker plugins and store them in an internal array.
    def locate_plugins
    ...
    # @return [Hash] A hash with all plugin names (minus the 'debunker-') as
