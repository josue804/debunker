class Debunker
  # Implements a hooks system for Debunker. A hook is a callable that is associated
  # with an event. A number of events are currently provided by Debunker, these
  # include: `:when_started`, `:before_session`, `:after_session`.  A hook must
  # have a name, and is connected with an event by the `Debunker::Hooks#add_hook`
  # method.
  #
  # @example Adding a hook for the `:before_session` event.
  #   Debunker.config.hooks.add_hook(:before_session, :say_hi) do
  #     puts "hello"
  #   end
  class Hooks
    def initialize
    ...
    # Destructively merge the contents of two `Debunker:Hooks` instances.
    #
    # @param [Debunker::Hooks] other The `Debunker::Hooks` instance to merge
    # @return [Debunker:Hooks] The receiver.
    # @see {#merge}
    def merge!(other)
    ...
    # @example
    #   hooks = Debunker::Hooks.new.add_hook(:before_session, :say_hi) { puts "hi!" }
    #   Debunker::Hooks.new.merge(hooks)
    # @param [Debunker::Hooks] other The `Debunker::Hooks` instance to merge
    # @return [Debunker::Hooks] a new `Debunker::Hooks` instance containing a merge of the
    #   contents of two `Debunker:Hooks` instances.
    def merge(other)
      self.dup.tap do |v|
        v.merge!(other)
      end
    ...
    #Alias for `add_hook`/`delete_hook` for that.
