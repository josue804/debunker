  ...
  module Helpers
    ...
    module CommandHelpers
      ...
      def internal_binding?(target)
        ...
        ["__binding__", "__debunker__", "class_eval"].include?(m)
      end

      def get_method_or_raise(name, target, opts={}, omit_help=false)
        meth = Debunker::Method.from_str(name, target, opts)
      ...
      def set_file_and_dir_locals(file_name, _debunker_=_debunker_(), target=target())
        return if !target or !file_name
        _debunker_.last_file = File.expand_path(file_name)
        _debunker_.inject_local("_file_", _debunker_.last_file, target)

        _debunker_.last_dir = File.dirname(_debunker_.last_file)
        _debunker_.inject_local("_dir_", _debunker_.last_dir, target)
      end
    end
