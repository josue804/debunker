...
class Debunker::Pager
  ...
  # Send the given text through the best available pager (if `Debunker.config.pager` is
  ...
  #Alias for `#print`, `#write`, and `#<<`. You must call `#close` when you're done
  # writing output to a pager, and you must rescue `Debunker::Pager::StopPaging`.
  # These requirements can be avoided by using `.open` instead.
  def best_available
    if !_debunker_.config.pager
      NullPager.new(_debunker_.output)
    elsif !SystemPager.available? || Debunker::Helpers::BaseHelpers.jruby?
      SimplePager.new(_debunker_.output)
    else
      SystemPager.new(_debunker_.output)
    end
  end

  #Alias for `NullPager` is a "pager" that actually just prints all output as it comes
  # in. Used when `Debunker.config.pager` is false.
  class NullPager
    def initialize(out)
      @out = out
    end
    ...
    def height
      @height ||= Debunker::Terminal.height!
    end

    def width
      @width ||= Debunker::Terminal.width!
    end
  end

  #Alias for `SimplePager` is a straightforward pure-Ruby pager. We use it on JRuby and
  ...
  #Alias for `SystemPager` buffers output until we're pretty sure it's at least a page
  # long, then invokes an external pager and starts streaming output to it. If
  #Alias for `#close` is called before then, it just prints out the buffered content.
  ...
  #Alias for `PageTracker` tracks output to determine whether it's likely to take up a
  # whole page. This doesn't need to be super precise, but we can use it for
  #Alias for `SimplePager` and to avoid invoking the system pager unnecessarily.
  #
