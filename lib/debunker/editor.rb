class Debunker
  class Editor
    ...
    def invoke_editor(file, line, blocking=true)
      raise CommandError, "Please set Debunker.config.editor or export $VISUAL or $EDITOR" unless _debunker_.config.editor
    ...
    def build_editor_invocation_string(file, line, blocking)
      ...
      if _debunker_.config.editor.respond_to?(:call)
        args = [file, line, blocking][0...(_debunker_.config.editor.arity)]
        _debunker_.config.editor.call(*args)
      else
        ...
        "#{_debunker_.config.editor} #{blocking_flag_for_editor(blocking)} #{start_line_syntax_for_editor(sanitized_file, line)}"
      end
    end

    # Start the editor running, using the calculated invocation string
    def open_editor(editor_invocation)
      # Note we dont want to use Debunker.config.system here as that
    ...
    # not to block the process from which they were launched (in this case, Debunker).
    ...
    # Get the name of the binary that Debunker.config.editor points to.
    #
    ...
    #
    # @example
    #   Debunker.config.editor="/home/conrad/bin/textmate -w"
    #   editor_name
    #   # => textmate
    #
    def editor_name
      File.basename(_debunker_.config.editor).split(" ").first
    end
