class Debunker
  class Method
    class Patcher
      ...
      def cache_key
        "debunker-redefined(0x#{method.owner.object_id.to_s(16)}##{method.name})"
      end
      ...
      def with_method_transaction
        temp_name = "__debunker_#{method.original_name}__"
        method = self.method
      ...
      def wrap_for_nesting(source)
        nesting = Debunker::Code.from_file(method.source_file).nesting_at(method.source_line)

        (nesting + [source] + nesting.map{ "end" } + [""]).join(";")
      rescue Debunker::Indent::UnparseableNestingError
        source
      end
    end
  end
