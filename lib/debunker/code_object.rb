class Debunker
  ...
  #Alias for `Debunker::Command` will be returned. Alternatively, if the user passes in "Debunker#repl" then
  # a `Debunker::Method` object will be returned.
  #
  ...
  class CodeObject
    module Helpers
      # we need this helper as some Debunker::Method objects can wrap Procs
      # @return [Boolean]
      ...
      def command?
        is_a?(Module) && self <= Debunker::Command
      end
    ...
    class << self
      def lookup(str, _debunker_, options={})
        co = new(str, _debunker_, options)
    ...
    def initialize(str, _debunker_, options={})
      options = {
        :super => 0,
      }.merge!(options)
    ...
    def command_lookup
      ...
      # raise?
      _debunker_.commands.find_command_by_match_or_listing(str) rescue nil
    end
    ...
    def empty_lookup
      ...
      obj = if internal_binding?(target)
              ...
              Debunker::WrappedModule(mod)
            else
              Debunker::Method.from_binding(target)
            end
    ...
    def default_lookup
      ...
      if safe_to_evaluate?(str) && !looks_like_an_instance_method?(str)
        ...
        if sourcable_object?(obj)
          ...
          Debunker::WrappedModule(obj.class)
        else
          nil
        end
      end
    ...
    def method_or_class_lookup
      obj = case str
            when /\S+\(\)\z/
              Debunker::Method.from_str(str.sub(/\(\)\z/, ''),target) || Debunker::WrappedModule.from_str(str, target)
            else
              Debunker::WrappedModule.from_str(str,target) || Debunker::Method.from_str(str, target)
            end
    ...
    # TODO: see duplication of this method in Debunker::WrappedModule
    ...
    def lookup_super(obj, super_level)
      ...
      if !sup
        raise Debunker::CommandError, "No superclass found for #{obj.wrapped}"
      else
        sup
      end
    end
