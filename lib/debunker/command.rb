require 'delegate'
require 'debunker/helpers/documentation_helpers'

class Debunker

  # The super-class of all commands, new commands should be created by calling
  # {Debunker::CommandSet#command} which creates a BlockCommand or {Debunker::CommandSet#create_command}
  ...
  class Command
    ...
    class << self
      ...
      def source
        ...
        strip_leading_whitespace(Debunker::Code.from_file(file).expression_at(line))
      end
    ...
    class << self
      def name
        super.to_s == "" ? "#<class(Debunker::Command #{match.inspect})>" : super
      end
      ...
      # @param [Hash] options Behavioral options (see {Debunker::CommandSet#command})
      # @param [Module] helpers A module of helper functions to be included.
      # @yield optional, used for BlockCommands
      # @return [Class] (a subclass of {Debunker::Command})
      ...
      # @deprecated Replaced with {Debunker::Hooks#add_hook}. Left for compatibility.
      # Store hooks to be run before or after the command body.
      # @see {Debunker::CommandSet#before_command}
      # @see {Debunker::CommandSet#after_command}
      def hooks
        Debunker.hooks
      end

      def command_regex
        pr = Debunker.respond_to?(:config) ? Debunker.config.command_prefix : ""
      ...
      def group(name=nil)
        @group ||= if name
                     ...
                     case Debunker::Method(block).source_file
                     when %r{/debunker/.*_commands/(.*).rb}
                       $1.capitalize.gsub(/_/, " ")
                     when %r{(debunker-\w+)-([\d\.]+([\w\.]+)?)}
                       name, version = $1, $2
    ...
    # Properties of one execution of a command (passed by {Debunker#run_command} as a hash of
    ...
    def run(command_string, *args)
      command_string = _debunker_.config.command_prefix.to_s + command_string
    ...
    include Debunker::Helpers::BaseHelpers
    include Debunker::Helpers::CommandHelpers
    ...
    def initialize(context={})
      ...
      self._debunker_        = context[:debunker_instance]
    end

    # @return [Object] The value of `self` inside the `target` binding.
    def target_self; target.eval('self'); end

    # @return [Hash] Debunker commands can store arbitrary state
    ...
    #                                #Alias for `state.my_state` used in another command.
    def state
      _debunker_.command_state[match] ||= Debunker::Config.from_hash({})
    end
    ...
    def check_for_command_collision(command_match, arg_string)
      ...
      if collision_type
        output.puts "#{text.bold('WARNING:')} Calling Debunker command '#{command_match}', which conflicts with a #{collision_type}.\n\n"
      end
    rescue Debunker::RescuableException
    end
    ...
    def process_line(line)
      ...
      check_for_command_collision(command_match, arg_string) if Debunker.config.collision_warning
    ...
    def pass_block(arg_string)
      ...
      if !Debunker::Code.complete_expression?(prime_string)
        block_string = _debunker_.r(target, prime_string)
      else
  ...
  #
  # Create subclasses using {Debunker::CommandSet#command}.
  ...
  # subclasses.
  #
  # Create subclasses using {Debunker::CommandSet#create_command}, and override the
  #Alias for `options(opt)` method to set up an instance of Debunker::Slop, and the `process`
  # method to actually run the command. If necessary, you can also override
  #Alias for `setup` which will be called before `options`, for example to require any
  ...
  class ClassCommand < Command
    class << self
      ...
      #
      # This should be a `Debunker::Method(block)` for a command made with `create_command`
      # and a `Debunker::WrappedModule(self)` for a command that's a standard class.
      # @return [Debunker::WrappedModule, Debunker::Method]
      def source_object
        @source_object ||= if name =~ /^[A-Z]/
                             Debunker::WrappedModule(self)
                           else
                             Debunker::Method(block)
                           end
      end
    end
    ...
    # Return the help generated by Debunker::Slop for this command.
    def help
      slop.help
    end

    # Return an instance of Debunker::Slop that can parse either subcommands or the
    # options that this command accepts.
    def slop
      Debunker::Slop.new do |opt|
    ...
    # A method to setup Debunker::Slop commands so it can parse the subcommands your
    ...
    #     #'Downloads a content from a server'
    #
    ...
    # A method to setup Debunker::Slop so it can parse the options your command expects.
    #
    # @note Please don't do anything side-effecty in the main part of this
    # method, as it may be called by Debunker at any time for introspection reasons.
    ...
    #
    # The `opts` mehod can be called to get the options that Debunker::Slop has passed,
