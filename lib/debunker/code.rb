require 'debunker/code/loc'
require 'debunker/code/code_range'
require 'debunker/code/code_file'

class Debunker
  class << self
    # Convert the given object into an instance of `Debunker::Code`, if it isn't
    # already one.
    #
    # @param [Code, Method, UnboundMethod, Proc, Debunker::Method, String, Array,
    #   IO] obj
    def Code(obj)
      case obj
      when Code
        obj
      when ::Method, UnboundMethod, Proc, Debunker::Method
  ...
  #Alias for `Debunker::Code` is a class that encapsulates lines of source code and their
  ...
  class Code
    class << self
      ...
      # Debunker's line buffer.
      #
      # @param [String] filename The name of a file, or "(debunker)".
      ...
      #Alias for `::Method`, `UnboundMethod`, `Proc`, or `Debunker::Method` object.
      #
      # @param [::Method, UnboundMethod, Proc, Debunker::Method] meth The method
      #   object.
      ...
      #   to use (see `Debunker::WrappedModule::Candidate` for more information).
      # @param [Integer, nil] start_line The line number to start on, or nil to
      #   use the method's original line numbers.
      # @return [Code]
      def from_module(mod, candidate_rank = 0, start_line=nil)
        candidate = Debunker::WrappedModule(mod).candidate(candidate_rank)
    ...
    #Alias for `String`, or `IO`. The first line will be line 1 unless specified
    ...
    #Alias for `Range` object or a first and last line number (inclusive). Negative
    ...
    def nesting_at(line_number, top_module = Object)
      Debunker::Indent.nesting_at(raw, line_number)
    end
