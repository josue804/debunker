class Debunker
  class Command::Whereami < Debunker::ClassCommand
  ...
  #'Show code surrounding the current context.'
    ...
    banner <<-'BANNER'
      ...
      Describe the current location. If you use `binding.debunker` inside a method then
      ...
      no code to show. This is used by debunker in the default before_session hook to show
      you when you arrive at a `binding.debunker`.

      The `-n` flag can be used to hide line numbers so that code can be copy/pasted
      effectively.

      When debunker was started on an Object and there is no associated method, whereami
    ...
    def setup
      ...
      @method = Debunker::Method.from_binding(target)
    end
    ...
    def handle_internal_binding
      if top_level?
        ...
        output.puts "Inside #{Debunker.view_clip(target_self)}."
      end
    end
    ...
    def code_window
      Debunker::Code.from_file(@file).around(@line, window_size)
    end
    ...
    def method_code
      ...
      if valid_method?
        @method_code = Debunker::Code.from_method(@method)
      end
    end

    # This either returns the `target_self`
    # or it returns the class of `target_self` if `target_self` is not a class.
    # @return [Debunker::WrappedModule]
    def target_class
      target_self.is_a?(Module) ? Debunker::WrappedModule(target_self) :
        Debunker::WrappedModule(target_self.class)
    end

    def class_code
      @class_code ||=
        begin
          mod = @method ? Debunker::WrappedModule(@method.owner) : target_class
          idx = mod.candidates.find_index { |v| expand_path(v.source_file) == @file }
          idx && Debunker::Code.from_module(mod, idx)
        end
    end
    ...
    def window_size
      if args.empty?
        _debunker_.config.default_window_size
      else
        args.first.to_i
      end
    end
  ...
  Debunker::Commands.add_command(Debunker::Command::Whereami)
  #Debunker::Commands.alias_command '@', 'whereami'
end
