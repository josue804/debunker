class Debunker
  class Command::ShowInfo < Debunker::ClassCommand
    extend Debunker::Helpers::BaseHelpers
    ...
    def process
      code_object = Debunker::CodeObject.lookup(obj_name, _debunker_, :super => opts[:super])
      ...
      if !obj_name && code_object.c_module? && !opts[:all]
        result = "Warning: You're inside an object, whose class is defined by means\n" +
                 "         of the C Ruby API. Debunker cannot display the information for\n" +
                 "         this class."
    ...
    def header(code_object)
      ...
      h = "\n#{Debunker::Helpers::Text.bold('From:')} #{file_name} "
      h << code_object_header(code_object, line_num)
      h << "\n#{Debunker::Helpers::Text.bold('Number of lines:')} " <<
    ...
    def code_object_header(code_object, line_num)
      if code_object.real_method_object?
        ...
        # It sucks we have to test for both Debunker::WrappedModule and WrappedModule::Candidate,
        # probably indicates a deep refactor needs to happen in those classes.
      elsif code_object.is_a?(Debunker::WrappedModule) || code_object.is_a?(Debunker::WrappedModule::Candidate)
    ...
    def show_all_modules?(code_object)
      code_object.is_a?(Debunker::WrappedModule) && opts.present?(:all)
    end
    ...
    def complete(input)
      if input =~ /([^ ]*)#([a-z0-9_]*)\z/
        prefix, search = [$1, $2]
        methods = begin
                    Debunker::Method.all_from_class(binding.eval(prefix))
