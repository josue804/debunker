class Debunker
  class Command::ShellCommand < Debunker::ClassCommand
    match(/\.(.*)/)
    ...
    def process(cmd)
      if cmd =~ /^cd\s*(.*)/i
        ...
        if command_block
          ...
          _debunker_.config.system.call(output, cmd, _debunker_)
        end
      end
    end
  ...
  Debunker::Commands.add_command(Debunker::Command::ShellCommand)
end
