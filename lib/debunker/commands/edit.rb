class Debunker
  class Command::Edit < Debunker::ClassCommand
    require 'debunker/commands/edit/exception_patcher'
    require 'debunker/commands/edit/file_and_line_locator'
  ...
  #'Invoke the default editor on a file.'
    ...
    banner <<-'BANNER'
      ...
      Open a text editor. When no FILE is given, edits the debunker input buffer.
      When a method/module/command is given, the code is opened in an editor.
      Ensure `Debunker.config.editor` or `_debunker_.config.editor` is set to your editor of choice.
      ...
      https://github.com/debunker/debunker/wiki/Editor-integration#wiki-Edit_command
    BANNER
    ...
    def process
      ...
      if repl_edit?
        # code defined in debunker, eval'd within debunker.
        repl_edit
      elsif runtime_patch?
    ...
    def repl_edit
      content = Debunker::Editor.new(_debunker_).edit_tempfile_with_content(initial_temp_file_content,
    ...
    def runtime_patch?
       !file_based_exception? && (opts.present?(:patch) || previously_patched?(code_object) || debunker_method?(code_object))
    end

    def apply_runtime_patch
      if patch_exception?
        ExceptionPatcher.new(_debunker_, state, file_and_line_for_current_exception).perform_patch
      else
        if code_object.is_a?(Debunker::Method)
          code_object.redefine Debunker::Editor.new(_debunker_).edit_tempfile_with_content(code_object.source)
        else
    ...
    def file_and_line_for_current_exception
      FileAndLineLocator.from_exception(_debunker_.last_exception, opts[:ex].to_i)
    end
    ...
    def file_edit
      ...
      Debunker::Editor.new(_debunker_).invoke_editor(file_name, line, reload?(file_name))
    ...
    def code_object
      @code_object ||= !probably_a_file?(filename_argument) &&
        Debunker::CodeObject.lookup(filename_argument, _debunker_)
    end

    def debunker_method?(code_object)
      code_object.is_a?(Debunker::Method) &&
        code_object.debunker_method?
    end

    def previously_patched?(code_object)
      code_object.is_a?(Debunker::Method) && Debunker::Method::Patcher.code_for(code_object.source_location.first)
    end
    ...
    def input_expression
      case opts[:i]
      when Range
        (_debunker_.input_array[opts[:i]] || []).join
      when Fixnum
        _debunker_.input_array[opts[:i]] || ""
      else
        raise Debunker::CommandError, "Not a valid range: #{opts[:i]}"
      end
    end
    ...
    def never_reload?
      opts.present?(:'no-reload') || _debunker_.config.disable_auto_reload
    end
    ...
    def initial_temp_file_content
      ...
      when eval_string.strip != ""
        ...
        _debunker_.input_array.reverse_each.find { |x| x && x.strip != "" } || ""
      end
    end
  ...
  Debunker::Commands.add_command(Debunker::Command::Edit)
end
