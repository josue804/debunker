class Debunker
  class Command::FindMethod < Debunker::ClassCommand
  ...
  #'Recursively search for a method within a Class/Module or the current namespace.'
    ...
    banner <<-'BANNER'
      ...
      # the Debunker namespace. Matches Debunker#repl, etc.
      find-method re Debunker

      # Find all methods that contain the code:
      # output.puts inside the Debunker namespace.
      find-method -c 'output.puts' Debunker
    BANNER
    ...
    def search_all_methods(namespace)
      ...
      recurse_namespace(namespace) do |klass|
        (Debunker::Method.all_from_class(klass) + Debunker::Method.all_from_obj(klass)).each do |method|
  ...
  Debunker::Commands.add_command(Debunker::Command::FindMethod)
end
