class Debunker
  class Command::CodeCollector
    ...
    def initialize(args, opts, _debunker_)
      @args = args
      @opts = opts
    ...
    def self.inject_options(opt)
      ...
      opt.on :o, :out,   "Select lines from Debunker's output result history. Takes an index or range",
      :optional_argument => true, :as => Range, :default => -5..-1 do |r|
        output_result_ranges << (r || (-5..-1))
      end
      opt.on :i, :in,    "Select lines from Debunker's input expression history. Takes an index or range",
      :optional_argument => true, :as => Range, :default => -5..-1 do |r|
    ...
    #Alias for `-d CODE_OBJECT` it returns the docs for that code object.
    #
    # @return [String]
    def content
      @content ||=
        begin
          ...
          content = case
                    when opts.present?(:o)
                      debunker_output_content
                    when opts.present?(:i)
                      debunker_input_content
                    when opts.present?(:d)
                      code_object_doc
                    else
    ...
    # The code object
    #
    # @return [Debunker::WrappedModule, Debunker::Method, Debunker::Command]
    def code_object
      Debunker::CodeObject.lookup(obj_name, _debunker_,  :super =>  opts[:super])
    end
    ...
    # The selected `_debunker_.output_array` as a string, as specified by
    # the `-o` switch.
    #
    # @return [String]
    def debunker_output_content
      debunker_array_content_as_string(_debunker_.output_array, self.class.output_result_ranges) do |v|
        _debunker_.config.gist.inspecter.call(v)
      end
    end

    # The selected `_debunker_.input_array` as a string, as specified by
    # the `-i` switch.
    #
    # @return [String]
    def debunker_input_content
      debunker_array_content_as_string(_debunker_.input_array, self.class.input_expression_ranges) { |v| v }
    end
    ...
    def debunker_array_content_as_string(array, ranges, &block)
      all = ''
