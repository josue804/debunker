...
class Debunker
  class Command::Ls < Debunker::ClassCommand
    class InstanceVars < Debunker::Command::Ls::Formatter
      ...
      def output_self
        ivars = if Object === @interrogatee
                  Debunker::Method.safe_send(@interrogatee, :instance_variables)
                else
                  [] #TODO: BasicObject support
                end
        kvars = Debunker::Method.safe_send(interrogatee_mod, :class_variables)
