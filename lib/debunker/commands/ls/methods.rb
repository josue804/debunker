...
class Debunker
  class Command::Ls < Debunker::ClassCommand
    class Methods < Debunker::Command::Ls::Formatter
      ...
      def output_self
        ...
        resolution_order.take_while(&below_ceiling).reverse.map do |klass|
          ...
          heading = "#{ Debunker::WrappedModule.new(klass).method_prefix }methods"
      ...
      def below_ceiling
        ceiling = if @quiet_switch
                    [Debunker::Method.safe_send(interrogatee_mod, :ancestors)[1]] +
                      _debunker_.config.ls.ceiling
                  elsif @verbose_switch
                    []
                  else
                    _debunker_.config.ls.ceiling.dup
                  end
