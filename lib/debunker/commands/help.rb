class Debunker
  class Command::Help < Debunker::ClassCommand
    match 'help'
    group 'Help'
  #'Show a list of commands or information about a specific command.'
    ...
    def display_index(groups)
      ...
      _debunker_.pager.page help_text.join("\n\n")
    end
    ...
    # @param [Array<Debunker::Command>] commands
    ...
    #
    # @param [Array<Debunker::Command>] commands The commands to sort
    # @return [Array<Debunker::Command>] commands sorted by listing name.
    ...
    def display_command(command)
      _debunker_.pager.page command.new.help
    end
    ...
    # @return [String]
    def normalize(key)
      key.downcase.gsub(/debunker\W+/, '')
    end

    def group_sort_key(group_name)
      [%w(Help The only command you need, amirite Editing Introspection Input_and_output Navigating_debunker Gems Basic Commands).index(group_name.gsub(' ', '_')) || 99, group_name]
    end
  end

  Debunker::Commands.add_command(Debunker::Command::Help)
end
