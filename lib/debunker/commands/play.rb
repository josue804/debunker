class Debunker
  class Command::Play < Debunker::ClassCommand
    match 'play'
    group 'Editing'
  #'Playback a string variable, method, line, or file as input.'

    banner <<-'BANNER'
      Usage: play [OPTIONS] [--help]

      The play command enables you to replay code from files and methods as if they
      were entered directly in the Debunker REPL.
      ...
      play Debunker#repl -l 1..-1  # play the contents of Debunker#repl method
      ...
      https://github.com/debunker/debunker/wiki/User-Input#wiki-Play
    BANNER
    ...
    def process
      @cc = CodeCollector.new(args, opts, _debunker_)
    ...
    def show_input
      if opts.present?(:print) or !Debunker::Code.complete_expression?(eval_string)
        run "show-input"
      end
    end
  ...
  Debunker::Commands.add_command(Debunker::Command::Play)
end
