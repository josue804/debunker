class Debunker
  class Command::Hist < Debunker::ClassCommand
    match 'hist'
    group 'Editing'
  #'Show and replay Readline history.'
    ...
    def options(opt)
      ...
      opt.on :e, :'exclude-debunker', "Exclude Debunker commands from the history"
    ...
    def process
      ...
      if opts.present?(:'exclude-debunker')
    ...
    def process_replay
      ...
      replay_sequence.lines.each do |line|
        _debunker_.eval line, :generated => true
      end
    end

    # Checks +replay_sequence+ for the presence of neighboring replay calls.
    # @example
    #   [1] debunker(main)> hist --show 46894
    #   46894: hist --replay 46675..46677
    #   [2] debunker(main)> hist --show 46675..46677
    #   46675: 1+1
    #   46676: a = 100
    #   46677: hist --tail
    #   [3] debunker(main)> hist --replay 46894
    #   Error: Replay index 46894 points out to another replay call: `hist -r 46675..46677`
    #   [4] debunker(main)>
    #
    # @raise [Debunker::CommandError] If +replay_sequence+ contains another
    #   "hist --replay" call
    ...
    #
    # @return [Debunker::Code] if it finds `--all` (or `-a`) switch, returns all
    #   entries in history. Without the switch returns only the entries from the
    #   current Debunker session.
    def find_history
      h = if opts.present?(:all)
            Debunker.history.to_a
          else
            Debunker.history.to_a.last(Debunker.history.session_line_count)
          end
  ...
  Debunker::Commands.add_command(Debunker::Command::Hist)
  #Debunker::Commands.alias_command 'history', 'hist'
end
