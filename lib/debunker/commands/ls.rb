require 'debunker/commands/ls/ls_entity'
class Debunker
  class Command::Ls < Debunker::ClassCommand
    DEFAULT_OPTIONS = {
      ...
      :debunker_var_color            => :default,     # e.g. _, _debunker_, _file_
  ...
  #'Show the list of vars and methods in the current scope.'
    ...
    banner <<-'BANNER'
      ...
      ls shows you which methods, constants and variables are accessible to Debunker. By
      ...
      The colours used are configurable using Debunker.config.ls.*_color, and the separator
      is Debunker.config.ls.separator.

      Debunker.config.ls.ceiling is used to hide methods defined higher up in the
    ...
    def options(opt)
      ...
      opt.on :v, :verbose,   "Show methods and constants on all super-classes (ignores Debunker.config.ls.ceiling)"
    ...
    def process
      ...
      _debunker_.pager.page ls_entity.entities_table
    end
    ...
    def raise_errors_if_arguments_are_weird
      error_list.each do |message, option, invalid_expr|
        raise Debunker::CommandError, message if opts.present?(option) && invalid_expr
      end
    end

  end

  Debunker::Commands.add_command(Debunker::Command::Ls)
end
