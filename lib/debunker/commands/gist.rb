class Debunker
  class Command::Gist < Debunker::ClassCommand
    match 'gist'
    group 'Misc'
  #'Upload code, docs, history to https://gist.github.com/.'
    ...
    def input_content
      ...
      CodeCollector.input_expression_ranges.each do |range|
        input_expressions = _debunker_.input_array[range] || []
        Array(input_expressions).each_with_index do |code, index|
          corrected_index = index + range.first
          if code && code != ""
            content << code
            if code !~ /;\Z/
              content << "#{comment_expression_result_for_gist(_debunker_.config.gist.inspecter.call(_debunker_.output_array[corrected_index]))}"
            end
          end
        end
      end
    ...
    def gist_content(content, filename)
      response = ::Gist.gist(content, :filename => filename || "debunker_gist.rb", :public => !!opts[:p])
      if response
  ...
  Debunker::Commands.add_command(Debunker::Command::Gist)
  #Debunker::Commands.alias_command 'clipit', 'gist --clip'
end
