class Debunker
  class Command::Cat
    class FileFormatter < AbstractFormatter
      ...
      def initialize(file_with_embedded_line, _debunker_, opts)
        ...
        @code_from_file = Debunker::Code.from_file(file_name)
      end

      def format
        set_file_and_dir_locals(file_name, _debunker_, _debunker_.current_context)
      ...
      def code_window_size
        _debunker_.config.default_window_size || 7
      end
