class Debunker
  class Command::Cat
    class ExceptionFormatter < AbstractFormatter
      ...
      def initialize(exception, _debunker_, opts)
        @ex = exception
        @opts = opts
      ...
      def format
        check_for_errors
        set_file_and_dir_locals(backtrace_file, _debunker_, _debunker_.current_context)
        code = decorate(Debunker::Code.from_file(backtrace_file).
      ...
      def code_window_size
        _debunker_.config.default_window_size || 5
      end
