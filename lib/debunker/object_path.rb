class Debunker
  #Alias for `ObjectPath` implements the resolution of "object paths", which are strings
  ...
  # @see https://github.com/debunker/debunker/wiki/State-navigation
  class ObjectPath
    SPECIAL_TERMS = ["", "::", ".", ".."]
    ...
    def resolve
      ...
      begin
        ...
        when ".."
          ...
          stack.push(Debunker.binding_for(stack.last.eval(next_segment)))
        end
    ...
    def complete?(segment)
      SPECIAL_TERMS.include?(segment) || Debunker::Code.complete_expression?(segment)
    end
